#!/usr/bin/env python3
"""
Setup script that creates the cycle detector files
Run this first to create cycle_detector.py and test_detector.py
"""

import os

# Main cycle detector code
cycle_detector_code = '''#!/usr/bin/env python3
"""
...
# Test script code
test_detector_code = '''#!/usr/bin/env python3
"""
Quick test script for the Cycle-Aware Pattern Detector
"""

from cycle_detector import CycleAwarePatternDetector
import pandas as pd

# Initialize detector with default settings
detector = CycleAwarePatternDetector(cycle_length=28, min_games=30)

# Load your data
print("Loading WNBA game logs...")
detector.load_data('data/wnba_combined_gamelogs.csv')

# Quick test on a few top players
print("\\nRunning quick analysis on sample players...")

# Get top scorers for testing
top_scorers = detector.df.groupby('PLAYER_NAME')['PTS'].mean().nlargest(10).index.tolist()

print(f"\\nAnalyzing top {len(top_scorers)} scorers:")
for player in top_scorers[:3]:  # Test first 3
    print(f"\\n{'='*50}")
    print(f"Analyzing: {player}")
    print('='*50)
    
    result = detector.analyze_player(player)
    if result:
        print(f"Total games: {result['total_games']}")
        print(f"Date range: {result['date_range']}")
        print(f"Cycle consistency score: {result['cycle_consistency_score']:.3f}")
        ...
        for metric, data in result['metrics'].items():
            if data['classification'] != 'Insufficient Data':
                ...
                print(f"  - Dips detected: {data['dip_count']}")
                print(f"  - Avg drop magnitude: {data.get('avg_magnitude', 0):.3f}")
                if data.get('optimal_cycle_length'):
                    print(f"  - Optimal cycle: {data['optimal_cycle_length']} days")

# Run full analysis if you want
response = input("\\nRun full analysis on all players? (y/n): ")
if response.lower() == 'y':
    print("\\nRunning full analysis...")
    results = detector.analyze_all_players()
    detector.export_results(results, 'cycle_analysis_results.csv')
    
    # Show summary
    print("\\nAnalysis Summary:")
    print(f"Players with strong patterns (score > 0.7): {len([r for r in results if r['cycle_consistency_score'] > 0.7])}")
    print(f"Players with moderate patterns (0.5-0.7): {len([r for r in results if 0.5 <= r['cycle_consistency_score'] <= 0.7])}")
    print(f"Players with weak/no patterns: {len([r for r in results if r['cycle_consistency_score'] < 0.5])}")
'''

# Create the files
print("Creating cycle detector files...")

with open('cycle_detector.py', 'w', encoding='utf-8') as f:
    f.write(cycle_detector_code)
print("[SUCCESS] Created cycle_detector.py")

with open('test_detector.py', 'w', encoding='utf-8') as f:
    f.write(test_detector_code)
print("[SUCCESS] Created test_detector.py")

print("\n[COMPLETE] Setup complete! You can now run:")
print("   python cycle_detector.py data/wnba_combined_gamelogs.csv")
