#!/usr/bin/env python3
"""
TARS Enhanced Menstrual Phase Estimator V2 - CORRECTED VERSION
Fixed for uppercase columns and date format issues
"""
...
def add_menstrual_intelligence(df, player_id_col='PLAYER_ID', date_col='GAME_DATE'):
    ...
    # Ensure date column is datetime - handle various formats
    try:
    ...
    # Need to rename columns temporarily for the trend engine
    df_for_trends = df.copy()
    df_for_trends['player_id'] = df_for_trends[player_id_col]
    df_for_trends['game_date'] = df_for_trends[date_col]
    df_for_trends['actual_fantasy_points'] = df_for_trends['WNBA_FANTASY_PTS']
    df_for_trends['minutes'] = df_for_trends['MIN']
    
    # Process all players for trends
    processed_dfs = []
    for player_id in unique_players:
        player_data = df_for_trends[df_for_trends['player_id'] == player_id].copy()
        ...
        player_data = player_data.drop(['player_id', 'game_date', 'actual_fantasy_points', 'minutes'], axis=1)
...
def update_props_with_cycle_info(props_df, gamelogs_df):
    ...
    if 'game_date' in props_df.columns:
        props_df['game_date'] = pd.to_datetime(props_df['game_date'])
    elif 'GAME_DATE' in props_df.columns:
        props_df['GAME_DATE'] = pd.to_datetime(props_df['GAME_DATE'])
    
    gamelogs_df['GAME_DATE'] = pd.to_datetime(gamelogs_df['GAME_DATE'])
    
    # Check which columns exist in props
    player_col = 'PLAYER_ID' if 'PLAYER_ID' in props_df.columns else 'player_id'
    date_col = 'GAME_DATE' if 'GAME_DATE' in props_df.columns else 'game_date'
    ...
    # Rename columns to match props if needed
    merge_df = gamelogs_df[cycle_cols].copy()
    if player_col == 'player_id':
        merge_df['player_id'] = merge_df['PLAYER_ID']
    if date_col == 'game_date':
        merge_df['game_date'] = merge_df['GAME_DATE']
